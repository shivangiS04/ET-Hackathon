...
class BatteryService:
    ...
    def predict_soh(
        ...
        charge_history = [
            charge.model_dump() if hasattr(charge, "model_dump")
            else charge.dict() if hasattr(charge, "dict")
            else charge
        ...
        ambient_temp_stress = max(0, (ambient_temp_c - 35)) * 0.01
        total_degradation = (
            ...
            ambient_temp_stress                  # 10% from ambient temp
        )
        ...
        soh = min(BatteryService.SOH_MAX, max(BatteryService.SOH_MIN, soh))
        ...
        return {
            ...
            "degradation_breakdown": {
                ...
                "ambient_temperature": round(ambient_temp_stress, 2)
            },
    ...
    def calculate_fleet_statistics(vehicles: List[Dict]) -> Dict:
        ...
        return {
            ...
            "vehicles_high_risk": sum(1 for s in sohs if s < 70),
            "vehicles_medium_risk": sum(1 for s in sohs if 70 <= s < 85),
            "vehicles_healthy": sum(1 for s in sohs if s >= 85),
        }
